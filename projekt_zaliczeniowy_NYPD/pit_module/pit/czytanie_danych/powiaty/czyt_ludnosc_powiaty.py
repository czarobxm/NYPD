...
def czyt_ludnosc_powiaty(ludnosc_w_powiatach_path):
    col_names = ['specyfikacja', 'identyfikator', 'ogółem', 'mężczyźni', 'kobiety', 'miasta']
    if os.path.exists(ludnosc_w_powiatach_path):
        sheets_dict = pd.read_excel(ludnosc_w_powiatach_path, usecols=list(range(3)), names = col_names, skiprows=list(range(7)), sheet_name=None) # wczytywanie pliku excel
    else:
        return pd.DateFrame()
    # wszystkie sheet'sy wrzucamy do jednego dateframe'a
    ludnosc_w_powiatach = pd.DataFrame() # do full table zapiszemy wszystkie informacje zawarte w kolejnych sheetsach
    for name, sheet in sheets_dict.items():
        sheet['sheet'] = name
        sheet = sheet.rename(columns=lambda x: x.split('\n')[-1])
        ludnosc_w_powiatach = ludnosc_w_powiatach.append(sheet)

    ludnosc_w_powiatach = ludnosc_w_powiatach.reset_index(drop=True)
    ...
    for i in ludnosc_w_powiatach.index:
        # zapisywanie identyfikatora
        try:
            int(ludnosc_w_powiatach.loc[i,'identyfikator'])
        except:
            is_id=False
        else:
            is_id = True
        ...
        if(is_id):
            id = str(ludnosc_w_powiatach.loc[i,'identyfikator'])
        # zapisywanie ludnosci wieku produkcyjnego dla identyfikatora
        if(ludnosc_w_powiatach.loc[i,'specyfikacja'] == 'Wiek  produkcyjny'):
            wiek_produkcyjny = ludnosc_w_powiatach.loc[i,'ogółem']
        # zapisywanie ludnosci wieku poprodukcyjnego dla identyfikatora
        if(ludnosc_w_powiatach.loc[i,'specyfikacja'] == 'Wiek poprodukcyjny'):
            wiek_poprodukcyjny = ludnosc_w_powiatach.loc[i,'ogółem']
